...
do
	...
	local BASE_MKDIR_MODE = bit.bor(
		...
		bit.bor(
			...
			bit.bor(
				bit.lshift(1, 6),
				bit.bor(bit.lshift(1, 5), bit.bor(bit.lshift(1, 3), bit.bor(bit.lshift(1, 2), bit.lshift(1, 0))))
			)
		)
	)
	...
	function Fs.mk_parent_dir_if_missing(path, success_callback)
		...
		Fs.exists(parent, function(exists, err)
			...
			elseif exists then
				...
				Fs.mk_parent_dir_if_missing(parent, function()
...
do
	...
	local BASE_OPEN_READ_MODE = bit.bor(bit.lshift(1, 8), bit.bor(bit.lshift(1, 5), bit.lshift(1, 2)))
...
do
	...
	local BASE_OPEN_WRITE_MODE =
		bit.bor(bit.lshift(1, 8), bit.bor(bit.lshift(1, 7), bit.bor(bit.lshift(1, 5), bit.lshift(1, 2))))
	...
	function Fs.write(path, content, opts)
		if Table.is_empty(opts) then
			opts = DEFAULT_WRITE_OPTS
		end
		--- @cast opts -nil

		--- @async
		local function write()
			open_and_write(path, content, opts.silent)
		end
